...
from app.models import Calculate, AvgScore
...
def visitorlogin(request):
    if request.method == 'POST':
        ...
        if request.POST.get('Generate') == 'Generate':
            ...
            res= cursor.fetchall()
            return render(request,'genrebased.html',{'query':res})
        
    else:
        return render(request,'genrebased.html')
...
def labels():
    labels = ["Comedy", "Thriller", "Drama", "Mystery","Action", "Romance", "Horror", "War", "Fantasy"]
...
def graph(request):
    ...
    iLabels = labels()
    for i in list:
        vote_count = AvgScore.objects.filter(mgenre__contains=i).count()
